...
function processEdge(element, dir){
  ...
  if (turn % 2 === 0){
    // element.innerHTML += `<div class='${dir}-teal'></div>`;
    if(edgeAlreadyTaken) return turn++;
    newEdge.classList.add(`${dir}-teal`);
    element.appendChild(newEdge);
  }
  else{
    // element.innerHTML += `<div class='${dir}-purple'></div>`;
    if(edgeAlreadyTaken) return turn++;
    newEdge.classList.add(`${dir}-purple`);
    element.appendChild(newEdge);
  }
...
function updateScores(inc){
  if (turn-- % 2 === 0) {
    tealScore+=inc;
    document.querySelector('.tealscore').classList.add('animate');
    document.querySelector('.tealscore').addEventListener('animationend', function() {
        this.classList.remove('animate');
    });
  }
  else {
    purpleScore+=inc;
    document.querySelector('.purplescore').classList.add('animate');
    document.querySelector('.purplescore').addEventListener('animationend', function() {
        this.classList.remove('animate');
    });
  }
  console.log("turn: "+turn);
  document.getElementById('tealscore').innerHTML = tealScore;
  document.getElementById('purplescore').innerHTML = purpleScore;
  if (edges.length === 2*2*SIDES*(SIDES-1)){
    if (purpleScore > tealScore) purpleWins();
    else if (purpleScore < tealScore) tealWins();
    else itsATie();
  }
}
